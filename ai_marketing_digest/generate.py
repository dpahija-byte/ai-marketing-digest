...
def _public_article_system_prompt(config: AppConfig) -> str:
    return (
        ...
        "- Use only sources provided in this run. Never cite sites, people, studies, or URLs that are not in the input.\n"
        "- Every number, percentage, statistic, or quote must come from the provided source material. If unsure, remove it.\n"
        "- Never invent plausible-sounding data. Prefer qualitative language when the source support is not precise.\n"
        "- Do not put words in a person's mouth. Any attributed quote must exist in the provided material.\n"
        "- No quote may exceed 15 words, and there should be no more than one short quote from any one source.\n"
        "- Every factual claim must be traceable to the provided source list. Unsupported claims must become clear opinion or be removed.\n"
        "- Do not use long dashes. Use commas, periods, or colons instead.\n"
        "- Do not open with generic phrases like 'In today's world' or 'In the age of AI'. Open with a concrete claim.\n"
        "- Before finalizing, run an internal publication check. If the article would fail, fix it before returning JSON.\n"
